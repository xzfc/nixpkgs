...
stdenv.mkDerivation (finalAttrs: {
  ...
  src = fetchurl {
    ...
    url = "https://installer.repos.intel.com/oneapi/vtune/lin/intel.oneapi.lin.vtune,v=${finalAttrs.version}.1%2B5/cupPayload.cup";
    hash = "sha256-wRZvR+gkJPHrJccbvdXBRocrjCi80oVAzkPe1C3ciyQ=";
  };
  ...
  autoPatchelfIgnoreMissingDeps = [
    "libffi.so.6" # Wanted by vendored python
    "libgdbm.so.4" # Wanted by vendored python
    "libgdbm_compat.so.4" # Wanted by vendored python
    "libreadline.so.6" # Wanted by vendored python
    "libopencl-clang.so.14" # Wanted by bin64/gma/GTPin/Profilers/ocloc/Bin/intel64/libigdfcl.so
    "libsycl.so.8" # Wanted by bin64/self_check_apps/matrix.dpcpp/matrix.dpcpp
  ];
